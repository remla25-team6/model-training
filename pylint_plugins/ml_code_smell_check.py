"""
This module defines a custom Pylint checker for detecting common code smells 
in machine learning code, particularly with Pandas and NumPy usage.
It checks for:
...
class MLCodeSmellChecker(BaseChecker):
    ...
    msgs = {
        "C9001": (
            "Avoid unnecessary iteration over Pandas DataFrame rows. "
        ...
        "C9003": (
            "Use df.to_numpy() instead of df.values for DataFrame to ndarray "
            "conversion.",
            "dataframe-conversion-misused",
            "df.to_numpy() is the preferred method over df.values.",
        ),
        "C9004": (
            "Use np.matmul() for matrix multiplication instead of "
            "np.dot() for clarity.",
    ...
    def visit_for(self, node: astroid.nodes.For) -> None:
        # Detect unnecessary iteration over DataFrame rows,
        # e.g., for _, row in df.iterrows()
        iter_call = node.iter
        if (
